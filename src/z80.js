...
export default function buildZ80(opts = {}) {
  ...
  const isBigEndian = (endianTestUint8[0] == 0x01);
  ...
  const registerPairIndexes = {
    'IX': 8, 'IY': 9
  }

  var rA, rF, rB, rC, rD, rE, rH, rL, rA_, rF_, rB_, rC_, rD_, rE_, rH_, rL_, rIXH, rIXL, rIYH, rIYL, rI, rR;
  var registerIndexes;
  ...
  */
  const getParamBoilerplate = (param, hasIXOffsetAlready = false) => {
    var match;

    if (param.match(/^[AFBCDEHL]|I[XY][HL]$/)) {
      const regNum = registerIndexes[param]
      return {
        'getter': '',
        'v': `regs[${regNum}]`,
        'trunc': '',
      ...
      const rp = registerPairIndexes[match[1]]
      var getter;
  ...
  */
  const ADC_A = (param) => {
    const operand = getParamBoilerplate(param);
    return `
      ${operand.getter}
  ...
  const BIT_N_iRRpNNi = (bit, rp) => { // requires 'offset'
    var updateSignFlag;
    if (bit == 7) {
  ...
  const CALL_C_NN = (flag, sense) => {
    var condition = `regs[${rF}] & ${flag}`;
    if (!sense) {
      condition = `!(${condition})`;
    }
    return `
  ...
  const CPDR = () => CPIR_CPDR('--');

  const CPIR = () => CPIR_CPDR('++');


  const DAA = () => {
    const subClause = SUB_A('add')
    const addClause = ADD_A('add')
    return `
      var add = 0;
  ...
  const INIR = () => INIR_INDR('+');

  const INDR = () => INIR_INDR('-');

  const JP_C_NN = (flag, sense) => {
    var condition = `regs[${rF}] & ${flag}`;
    if (!sense) {
      condition = `!(${condition})`;
    }
  ...
  const LD_iRRi_R = (rp, r) => {
    return `
      WRITEMEM(regPairs[${rp}], regs[${r}]);
    `;
  };

  const LD_iRRpNNi_N = (rp) => {
    return `
  ...
  const LD_R_iRRi = (r, rp) => {
    return `
      regs[${r}] = READMEM(regPairs[${rp}]);
    `;
  };

  const LD_R_iRRpNNi = (r, rp) => {
    return `
      var offset = READMEM(regPairs[${rpPC}]);
      CONTEND_READ_NO_MREQ(regPairs[${rpPC}], 1);
      CONTEND_READ_NO_MREQ(regPairs[${rpPC}], 1);
  ...
  const LDBITOP = (regName, opcode, bit, rp) => {
    // load (rp+nn) into register regName, perform opcode, write back to (rp+nn)
    const regNum = registerIndexes[regName]
    return `
      var addr = (regPairs[${rp}] + offset) & 0xffff;
  ...
  const LDIR = () => LDIR_LDDR('++');

  const LDDR = () => LDIR_LDDR('--');

  const LDSHIFTOP = (regName, opcode, rp) => {
    // load (rp+nn) into register regName, perform opcode, write back to (rp+nn)
    const regNum = registerIndexes[regName]
    return `
      var addr = (regPairs[${rp}] + offset) & 0xffff;
  ...
  const OUTD = () => OUTI_OUTD('--');;

  const OUTI = () => OUTI_OUTD('++');;

  const OTDR = () => OTIR_OTDR('--');;

  const OTIR = () => OTIR_OTDR('++');;
  ...
  const RES = (bit, param) => {
    const operand = getParamBoilerplate(param, true);
    const hexMask = 0xff ^ (1 << bit);
    return `
      ${operand.getter}
      ${operand.v} &= ${hexMask};
      ${operand.setter}
  ...
  */
  const opcodeSwitch = (runStringTable, fallbackTable = {}, traps = []) => {
    const clauses = []
    for (let i = 0; i < 0x100; i++) {
      var runString = runStringTable[i];
  ...
  const generateddfdcbOpcodeSet = (prefix) => {
    var rp, rh, rl, rpn, rhn, rln;
    if (prefix == 'DDCB') {
      rp = rpIX
      rh = rIXH
      rl = rIXL
  ...
  const OPCODE_RUN_STRINGS_DDCB = generateddfdcbOpcodeSet('DDCB');
  const OPCODE_RUN_STRINGS_FDCB = generateddfdcbOpcodeSet('FDCB');

  // Generate the opcode runner lookup table for either the DD or FD set, acting on the
  // specified register pair (IX or IY)
  const generateddfdOpcodeSet = (prefix) => {
    var rp, rh, rl, rpn, rhn, rln;
    if (prefix == 'DD') {
      rp = rpIX;
      rh = rIXH;
  ...
  const OPCODE_RUN_STRINGS_DD = generateddfdOpcodeSet('DD');

  const OPCODE_RUN_STRINGS_ED = {
  ...
  const OPCODE_RUN_STRINGS_FD = generateddfdOpcodeSet('FD');

  const OPCODE_RUN_STRINGS = {
    0x00: NOP(),        // NOP
        ...
        if (iff1) {
          ...
          switch (im) {
            ...
            case 2:
              ...
              var l = READMEM(inttemp);
              inttemp = (inttemp+1) & 0xffff;
              var h = READMEM(inttemp);
              regPairs[${rpPC}] = (h<<8) | l;
              tstates += 7;
              break;
          }
        ...
        while (tstates < frameLength || opcodePrefix) {
          ...
          switch (lastOpcodePrefix) {
            case '':
              ...
              ${opcodeSwitch(OPCODE_RUN_STRINGS, undefined, opts.traps)}
              break;
            case 'CB':
              CONTEND_READ(regPairs[${rpPC}], 4);
              opcode = memory.read(regPairs[${rpPC}]); regPairs[${rpPC}]++;
