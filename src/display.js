import { MODEL_128K } from './spectrum';
...
export default class Display {
  constructor(opts) {
    ...
    opts.settings.checkerboardFilter.onChange.bind((newValue) => {
    ...
    this.startFrame = () => {
      this.nextEventTime = currentLineStartTime = TSTATES_UNTIL_ORIGIN - (TOP_BORDER_LINES * TSTATES_PER_SCANLINE) - (LEFT_BORDER_CHARS * TSTATES_PER_CHAR);
    ...
    this.doEvent = () => {
      ...
      beamX++;
      if (beamX < BEAM_X_MAX) {
        this.nextEventTime += TSTATES_PER_CHAR;
      } else {
        beamX = (border ? -LEFT_BORDER_CHARS : 0);
        beamY++;
        ...
        if (beamY < BEAM_Y_MAX) {
          ...
          this.nextEventTime = currentLineStartTime;
        } else {
    ...
    this.drawFullScreen = () => {
      this.startFrame();
      while (this.nextEventTime) this.doEvent();
      this.endFrame();
    };
